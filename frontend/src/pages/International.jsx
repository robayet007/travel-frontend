...
const International = () => {
  ...
  const fetchPackages = async () => {
    try {
      ...
      // Filter only international tour packages
      const internationalPackages = response.data.data.filter(
        pkg => pkg.category === 'international-tour'
      )
  ...
  return (
    <section className="min-h-screen px-4 py-16 mx-auto bg-center bg-no-repeat bg-cover max-w-7xl">
      ...
      {packages.length > 0 ? (
        <>
          <div className="mb-6 text-sm text-gray-600">
            Showing {packages.length} international tour package{packages.length !== 1 ? 's' : ''}
          </div>
      ...
      {/* Agency Info */}
      <div className="mt-[2rem] banner-img-container">
        <img
          src="/Bannar.jpg"
          alt="Agency Info"
